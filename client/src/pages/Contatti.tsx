import { motion } from "framer-motion";
import ContactForm from "@/components/ContactForm";
import AnimatedElement from "@/components/AnimatedElement";
import LandoReveal from "@/components/LandoReveal";
import StaggerContainer, { StaggerItem } from "@/components/StaggerContainer";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Mail, Phone, MapPin, Clock } from "lucide-react";
import { offices } from "@/lib/data";
import { useLanguage } from "@/lib/i18n";
import { useTheme } from "@/contexts/ThemeContext";

const themeOverlays: Record<string, { gradient: string; accent: string }> = {
  default: {
    gradient: 'linear-gradient(135deg, rgba(8, 57, 107, 0.88) 0%, rgba(10, 102, 194, 0.78) 50%, rgba(8, 57, 107, 0.92) 100%)',
    accent: '#7eb8e5',
  },
  cassazione: {
    gradient: 'linear-gradient(135deg, rgba(8, 57, 107, 0.88) 0%, rgba(10, 102, 194, 0.78) 50%, rgba(8, 57, 107, 0.92) 100%)',
    accent: '#7eb8e5',
  },
  colosseo: {
    gradient: 'linear-gradient(135deg, rgba(139, 69, 19, 0.88) 0%, rgba(180, 100, 50, 0.78) 50%, rgba(139, 69, 19, 0.92) 100%)',
    accent: '#e8b88a',
  },
  'villa-este': {
    gradient: 'linear-gradient(135deg, rgba(34, 85, 51, 0.88) 0%, rgba(60, 120, 70, 0.78) 50%, rgba(34, 85, 51, 0.92) 100%)',
    accent: '#90c090',
  },
  trevi: {
    gradient: 'linear-gradient(135deg, rgba(0, 100, 100, 0.88) 0%, rgba(32, 140, 140, 0.78) 50%, rgba(0, 100, 100, 0.92) 100%)',
    accent: '#80d4d4',
  },
  borghese: {
    gradient: 'linear-gradient(135deg, rgba(25, 80, 60, 0.88) 0%, rgba(50, 120, 90, 0.78) 50%, rgba(25, 80, 60, 0.92) 100%)',
    accent: '#88c8a8',
  },
};

export default function Contatti() {
  const { t, language } = useLanguage();
  const { currentTheme } = useTheme();
  const overlay = themeOverlays[currentTheme.id] || themeOverlays.default;
  
  return (
    <div className="relative">
      <div className="relative z-0 pt-24 pb-10 md:pb-16 text-white overflow-hidden min-h-[min(40vh,400px)]">
        <div 
          className="absolute inset-0 bg-cover bg-center"
          style={{ backgroundImage: `url(${currentTheme.heroImage})` }}
        />
        <div 
          className="absolute inset-0"
          style={{ background: '#2e6884e6' }}
        />
        <div className="w-full px-5 md:px-12 lg:px-16 relative z-10">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 md:gap-8">
            <div className="flex-1 max-w-xl">
              <AnimatedElement once={true}>
                <LandoReveal
                  text={t("contatti.title")}
                  as="h1"
                  className="text-2xl md:text-4xl lg:text-5xl mb-3 md:mb-4 text-left text-brutalist text-white"
                  delay={100}
                />
              </AnimatedElement>
              <AnimatedElement delay={200} once={true}>
                <p className="text-white/80 mb-3 md:mb-4 text-left text-[15px] md:text-lg">
                  {t("contatti.description")}
                </p>
              </AnimatedElement>
            </div>
          </div>
        </div>
      </div>
      
      <section className="py-10 md:py-16 relative z-10">
        <div className="w-full px-5 md:px-12 lg:px-16 relative z-10">
          <StaggerContainer staggerDelay={0.15} className="grid lg:grid-cols-3 gap-6 md:gap-8">
            <StaggerItem
              className="lg:col-span-2"
              y={30}
            >
              <ContactForm />
            </StaggerItem>

            <StaggerItem
              className="space-y-6"
              y={40}
            >
              <Card className="border-0 bg-card">
                <CardHeader>
                  <CardTitle className="text-xl">{t("contatti.directContact")}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <a
                    href="mailto:info@legalit.it"
                    className="flex items-center gap-4 p-3 rounded-lg bg-background hover:bg-primary/10 transition-colors"
                    data-testid="link-contact-email"
                  >
                    <div className="p-2 rounded-lg bg-primary/10">
                      <Mail className="h-5 w-5 text-primary" />
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground">Email</p>
                      <p className="font-medium">info@legalit.it</p>
                    </div>
                  </a>
                  <a
                    href="tel:063213911"
                    className="flex items-center gap-4 p-3 rounded-lg bg-background hover:bg-primary/10 transition-colors"
                    data-testid="link-contact-phone"
                  >
                    <div className="p-2 rounded-lg bg-primary/10">
                      <Phone className="h-5 w-5 text-primary" />
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground">{t("contatti.phone")}</p>
                      <p className="font-medium">06 3213911</p>
                    </div>
                  </a>
                  <div className="flex items-center gap-4 p-3 rounded-lg bg-background">
                    <div className="p-2 rounded-lg bg-primary/10">
                      <Clock className="h-5 w-5 text-primary" />
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground">{t("contatti.hours")}</p>
                      <p className="font-medium">{t("contatti.hoursValue")}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>

              <Card className="border-0 bg-card">
                <CardHeader>
                  <CardTitle className="text-xl">{t("contatti.ourLocations")}</CardTitle>
                </CardHeader>
                <CardContent>
                  <ul className="space-y-2">
                    {offices.map((office) => (
                      <li key={office.id}>
                        <a
                          href={`https://www.google.com/maps/search/${encodeURIComponent(office.address + " " + office.city)}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-3 p-2 rounded-lg hover:bg-primary/10 transition-colors"
                          data-testid={`link-office-${office.city.toLowerCase()}`}
                        >
                          <MapPin className="h-4 w-4 text-primary flex-shrink-0" />
                          <div>
                            <p className="font-medium text-sm">{office.city}</p>
                            <p className="text-xs text-muted-foreground">{office.address}</p>
                          </div>
                        </a>
                      </li>
                    ))}
                  </ul>
                </CardContent>
              </Card>
            </StaggerItem>
          </StaggerContainer>
        </div>
      </section>
    </div>
  );
}
