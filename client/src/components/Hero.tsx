...
export default function Hero({ loadingComplete = false }: HeroProps) {
  ...
  const accentWord4Ref = useRef<HTMLSpanElement>(null);
  ...
  useLayoutEffect(() => {
    if (alreadyScrolledRef.current) {
      ...
      [accentWord1Ref, accentWord2Ref, accentWord3Ref, accentWord4Ref].forEach(ref => {
        if (ref.current) {
  ...
  const triggerHeroReveal = useCallback(() => {
    ...
    [accentWord1Ref, accentWord2Ref, accentWord3Ref, accentWord4Ref].forEach((ref, wordIdx) => {
      if (ref.current) {
    ...
    <div className="relative" style={{ height: "100vh" }}>
      ...
      >
        ...
        >
          ...
          >
            <span className="text-white">Il tuo </span><span ref={accentWord1Ref} className="italic" style={{ color: '#003767', WebkitTextStroke: '5px white', paintOrder: 'stroke fill' }}>{Array.from("partner").map((ch, i) => <span key={i} className="accent-letter" style={{ display: 'inline-block', position: 'relative', zIndex: 1, opacity: 0 }}>{ch}</span>)}</span><br />
            <span className="text-white">per difendere i </span><span ref={accentWord2Ref} className="italic" style={{ color: '#003767', WebkitTextStroke: '5px white', paintOrder: 'stroke fill' }}>{Array.from("diritti").map((ch, i) => <span key={i} className="accent-letter" style={{ display: 'inline-block', position: 'relative', zIndex: 1, opacity: 0 }}>{ch}</span>)}</span><br />
            <span className="text-white">prevenire i </span><span ref={accentWord3Ref} className="italic" style={{ color: '#003767', WebkitTextStroke: '5px white', paintOrder: 'stroke fill' }}>{Array.from("rischi").map((ch, i) => <span key={i} className="accent-letter" style={{ display: 'inline-block', position: 'relative', zIndex: 1, opacity: 0 }}>{ch}</span>)}</span><br />
            <span className="text-white">e creare </span><span ref={accentWord4Ref} className="italic" style={{ color: '#003767', WebkitTextStroke: '5px white', paintOrder: 'stroke fill' }}>{Array.from("valore").map((ch, i) => <span key={i} className="accent-letter" style={{ display: 'inline-block', position: 'relative', zIndex: 1, opacity: 0 }}>{ch}</span>)}</span>
          </h1>
