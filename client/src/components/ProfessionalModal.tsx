...
function renderBio(bio: string) {
  ...
  return paragraphs.map((para, pi) => {
    ...
    if (isBulletBlock) {
      return (
        <ul key={pi} className="list-none space-y-1 mb-3">
          {lines.map((line, li) => {
            ...
            if (trimmed.startsWith('•')) {
              return (
                <li key={li} className="flex gap-2">
                  ...
                  <span dangerouslySetInnerHTML={{ __html: parseBoldMarkdown(trimmed.slice(1).trim()) }} />
                </li>
              );
            }
            return trimmed ? (
              <li key={li}>
                <span dangerouslySetInnerHTML={{ __html: parseBoldMarkdown(trimmed) }} />
              </li>
            ) : null;
          })}
        </ul>
    ...
    return (
      <p key={pi} className="mb-3 last:mb-0" dangerouslySetInnerHTML={{ __html: parseBoldMarkdown(para) }} />
    );
  });
}
          ...
          <div className="flex-1 min-h-0">
            <div className="p-4 md:p-8 pb-8 md:pb-12 space-y-5 md:space-y-6">
              ...
              >
                ...
                <div className="text-muted-foreground leading-relaxed pr-2">
                  {professional.fullBio ? renderBio(autoT(professional.fullBio)) : ""}
                </div>
              </motion.section>
