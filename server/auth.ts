...
export async function setupAuth(app: Express) {
  ...
  app.post("/api/auth/reset-password", authRateLimiter, async (req, res) => {
    try {
      ...
      if (password.length < 12 || !/[A-Z]/.test(password) || !/[a-z]/.test(password) || !/[0-9]/.test(password) || !/[^A-Za-z0-9]/.test(password)) {
        return res.status(400).json({ message: "La password deve avere almeno 12 caratteri, una maiuscola, una minuscola, un numero e un simbolo" });
      }
  ...
  app.post("/api/auth/request-login-code", authRateLimiter, async (req, res) => {
    try {
      ...
      // Always return success to prevent email enumeration
      if (!user) {
        return res.json({ message: "Se l'email esiste nel sistema, riceverai un codice di accesso." });
      }
...
export async function setupAdminRoutes(app: Express) {
  ...
  // Reset user password (authenticated partners)
