      ...
      await client.query(
        `INSERT INTO news_articles (id, title, content, excerpt, category, image_url, image_position, image_zoom, read_time, author_id, author_name, created_at, updated_at, document_url, document_name, news_type, macro_category, micro_category, linkedin_url, linked_professional_id, linked_professional_ids, linked_practice_area, tags, linkedin_summary) 
         ...
         ON CONFLICT (id) DO UPDATE SET
           ...
           linked_professional_id = EXCLUDED.linked_professional_id,
...
export async function exportDbToDevData() {
  ...
  try {
    const dataPath = path.join(process.cwd(), "server", "dev_data.json");
    if (!fs.existsSync(dataPath)) {
      console.error("[Export] dev_data.json not found at", dataPath);
      return;
    }

    const currentData: DevData = JSON.parse(fs.readFileSync(dataPath, "utf-8"));

    const profResult = await client.query(
      "SELECT row_to_json(p) AS data FROM (SELECT id, name, title, office, email, phone, full_bio, education, languages, image_url, image_position, image_zoom, order_index, created_at, updated_at, specializations, role, pec FROM professionals ORDER BY order_index) p"
    );
    const articlesResult = await client.query(
      "SELECT row_to_json(a) AS data FROM (SELECT id, title, content, excerpt, category, image_url, image_position, image_zoom, read_time, author_id, author_name, created_at, updated_at, document_url, document_name, news_type, macro_category, micro_category, linkedin_url, linked_professional_id, linked_professional_ids, linked_practice_area, tags, linkedin_summary FROM news_articles ORDER BY id) a"
    );
    const categoriesResult = await client.query(
      "SELECT row_to_json(c) AS data FROM (SELECT id, name, name_en, type, parent_category, is_default, created_at FROM news_categories ORDER BY id) c"
    );
    const settingsResult = await client.query(
      "SELECT row_to_json(s) AS data FROM (SELECT key, value, updated_at FROM site_settings ORDER BY key) s"
    );
    const subscribersResult = await client.query(
      "SELECT row_to_json(n) AS data FROM (SELECT id, email, subscribed_at, unsubscribed_at, source FROM newsletter_subscribers ORDER BY id) n"
    );

    currentData.professionals = profResult.rows.map((r: any) => r.data);
    currentData.news_articles = articlesResult.rows.map((r: any) => r.data);
    currentData.news_categories = categoriesResult.rows.map((r: any) => r.data);
    currentData.site_settings = settingsResult.rows.map((r: any) => r.data);
    currentData.newsletter_subscribers = subscribersResult.rows.map((r: any) => r.data);

    fs.writeFileSync(dataPath, JSON.stringify(currentData, null, 2), "utf-8");

    console.log(
      `[Export] Exported ${currentData.professionals.length} professionals, ${currentData.news_articles.length} articles, ${currentData.news_categories.length} categories, ${currentData.site_settings.length} settings to dev_data.json`
    );
  } catch (error) {
    console.error("[Export] Error exporting DB to dev_data.json:", error);
  } finally {
    client.release();
    await pool.end();
  }
